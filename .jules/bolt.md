...
## 2026-03-08 - [Between Rays Vectorization]\n**Learning:** The `between` function used in move generation and king safety checks evaluated to `case ray a b of 0 -> 0; bb -> bb \`clearBit\` (unSquare b)` dynamically at runtime. This added small but frequent branching and bitwise overhead in an inner hot loop (used by `givesCheckSlider`).\n**Action:** Replaced the runtime branch with a precomputed O(1) `U.unsafeIndex` unboxed vector lookup (`bbBetween`), saving per-node computation. Resulted in a ~5.3% NPS improvement on Start position (2.69M -> 2.84M) and a ~3.8% improvement on KiwiPete (44.3M -> 46.0M).\n

## 2026-03-09 - [Popcount replacement for Check Validation]
