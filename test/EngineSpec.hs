...
spec = do
  describe "Evaluation" $ do
    ...
    it "should value material" $ do
      ...
      let Just _board = parseFen "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1"
      evaluateSome (trustBoard board) `shouldBe` 0

      -- Position with material imbalance
      let Just _board = parseFen "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBN1 w KQkq - 0 1" -- White missing Rook at H1
    ...
    it "should recognize space/center control (somewhat)" $ do
       let Just _board = parseFen "rnbqkbnr/pppp1ppp/8/4p3/4P3/8/PPPP1PPP/RNBQKBNR w KQkq - 0 2"
       -- Both sides e4/e5. Equal.
       evaluateSome (trustBoard board) `shouldBe` 0

       let Just _board = parseFen "rnbqkbnr/pppppppp/8/8/4P3/8/PPPP1PPP/RNBQKBNR w KQkq - 0 1"
    ...
    it "should penalize king safety" $ do
       let Just _board = parseFen "rnbqkbnr/pppppppp/8/8/8/8/PPP1PPPP/RNBQKBNR w KQkq - 0 1" -- Normal
       evaluateSome (trustBoard board) `shouldSatisfy` (> (-50)) -- Roughly equal

       let Just _board = parseFen "rnbqkbnr/pppppppp/8/8/8/8/8/RNBQKBNR w KQkq - 0 1" -- White missing all pawns!
       evaluateSome (trustBoard board) `shouldSatisfy` (< (-500))

  describe "Search" $ do
    it "finds mate in 1 (Fool's Mate)" $ do
       let Just _board = parseFen "rnbqkbnr/pppp1ppp/8/4p3/6P1/5P2/PPPPP2P/RNBQKBNR b KQkq - 0 2"
       tt <- newTT 1
