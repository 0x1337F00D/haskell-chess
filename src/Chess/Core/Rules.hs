...
crazyhouseGameFromFEN s = do
  ...
  let c = case GS.turn gs of
      ...
      -- Promoted pieces are not standard FEN but some extensions might support it?
      -- The original code used Set.empty. We use 0.
      vs = CrazyhouseState wPocket bPocket 0

      checked = Val.isCheck baseBoard gs

      -- Check if any moves available (including drops)
      -- Use a simplified check for existence of moves
      hasMovesStd = Val.hasLegalMoves baseBoard gs

      -- Can drop?
      hasDrops :: forall col. KnownColor col => Bool
      hasDrops =
          let col = colorVal @col
              pocket = if col == White then wPocket else bPocket
              canDropAny = pocketPawns pocket > 0 || pocketKnights pocket > 0 || pocketBishops pocket > 0 || pocketRooks pocket > 0 || pocketQueens pocket > 0
              -- Need at least one empty square
              hasEmpty = BB.popcount (Base.occupiedTotal baseBoard) < 64
          in canDropAny && hasEmpty -- Rough check, exact check done in generateMoves

  case c of
      White -> if hasMovesStd || hasDrops @'White -- Approximation
               then if checked
      ...
      Black -> if hasMovesStd || hasDrops @'Black
               then if checked
