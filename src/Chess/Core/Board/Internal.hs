...
fromBaseBoard bb = do
  -- Validate Kings: Allow 0 or 1.
  -- Previously strictly 1. Now relaxed.
  if popCount (Base.whiteKings bb) > 1 then Nothing else return ()
  if popCount (Base.blackKings bb) > 1 then Nothing else return ()

  let wKingSq = if Base.whiteKings bb /= 0
                then Just (fromBaseSquare (T.Square (countTrailingZeros (Base.whiteKings bb))))
                else Nothing
  let bKingSq = if Base.blackKings bb /= 0
                then Just (fromBaseSquare (T.Square (countTrailingZeros (Base.blackKings bb))))
                else Nothing
