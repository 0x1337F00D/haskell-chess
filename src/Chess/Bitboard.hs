...
import Chess.Bitboard.Magic (Magic(..))
import qualified Chess.Bitboard.MagicTables as MT
...
bbBishopMagics = MT.bbBishopMagics

bbRookMagics :: U.Vector Magic
bbRookMagics = MT.bbRookMagics

bbMagicTable :: U.Vector Bitboard
bbMagicTable = MT.bbMagicTable
...
magicAttack (Magic mask magic sh offset) occ =
    let idx :: Int
        idx = fromIntegral (((occ .&. mask) * magic) `unsafeShiftR` sh)
    in bbMagicTable `U.unsafeIndex` (offset + idx)
