...
pack :: Color -> CastlingRights -> Square -> HalfmoveClock -> FullmoveNumber -> Word64
pack c cr ep hm fm =
    let crPacked = (cr .&. 0xFF) .|. ((cr `shiftR` 56) `shiftL` 8)
        cVal     = fromIntegral (fromEnum c)
        epVal    = case ep of NoSquare -> 64; Square s -> fromIntegral s
...
epSquare gs =
    ...
    in if val == 64 then NoSquare else Square (fromIntegral val)
...
setEpSquare ep gs =
    let val = case ep of NoSquare -> 64; Square s -> fromIntegral s
