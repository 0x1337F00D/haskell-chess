...
import Data.Word (Word64, Word16, Word32)
import Data.Bits (xor)
import qualified Data.Vector.Unboxed as V
import qualified Data.ByteString.Lazy as BL
import Data.Binary.Get (runGet, getWord64be, getWord16be, getWord32be, Get)
import System.IO (withBinaryFile, IOMode(ReadMode), hFileSize, hSeek, SeekMode(AbsoluteSeek), Handle)
...
polyglotKey b gs =
    let
        ...
        pieceKey = V.foldl' xor 0 $ V.fromList [
            ...
            | sq <- squares, pieceAt b sq /= Nothing
            ]
  ...
  where
    pieceRandom :: Maybe Piece -> Square -> Word64
    pieceRandom Nothing _ = 0
    pieceRandom (Just (Piece c pt)) sq =
        let
            -- black pawn 0, white pawn 1, black knight 2, ...
            pieceTypeIndex = fromEnum pt
            colorOffset = if c == White then 1 else 0
            kind = pieceTypeIndex * 2 + colorOffset
...
-- | Represents an entry in a Polyglot book.
...
-- | Read all book entries for a given key.
readPolyglotBook :: FilePath -> Word64 -> IO [BookEntry]
readPolyglotBook path key = do
    findPosition path key

-- | Find all entries matching the key using binary search.
findPosition :: FilePath -> Word64 -> IO [BookEntry]
findPosition path key = withBinaryFile path ReadMode $ \h -> do
    fileSize <- hFileSize h
    let entrySize = 16
    let numEntries = fileSize `div` entrySize

    -- Binary search for the first occurrence
    let
        search :: Integer -> Integer -> IO (Maybe Integer)
        search low high
            | low > high = return Nothing
            | otherwise = do
                let mid = (low + high) `div` 2
                hSeek h AbsoluteSeek (mid * entrySize)

                -- Read key (first 8 bytes)
                bytes <- BL.hGet h 8
                let midKey = runGet getWord64be bytes

                if midKey < key
                    then search (mid + 1) high
                    else if midKey > key
                        then search low (mid - 1)
                        else do
                            -- Found a match, but need the *first* match.
                            -- Check if previous entry also matches.
                            if mid == 0
                                then return (Just 0)
                                else do
                                    hSeek h AbsoluteSeek ((mid - 1) * entrySize)
                                    prevBytes <- BL.hGet h 8
                                    let prevKey = runGet getWord64be prevBytes
                                    if prevKey == key
                                        then search low (mid - 1)
                                        else return (Just mid)

    firstMatch <- search 0 (numEntries - 1)

    case firstMatch of
        Nothing -> return []
        Just idx -> readConsecutive h idx key entrySize

  where
    readConsecutive :: Handle -> Integer -> Word64 -> Integer -> IO [BookEntry]
    readConsecutive h idx targetKey entrySize = do
        hSeek h AbsoluteSeek (idx * entrySize)
        -- Read chunk
        bytes <- BL.hGet h 16
        if BL.length bytes < 16
            then return []
            else do
                let entry = runGet parseEntry bytes
                if bookKey entry == targetKey
                    then do
                        rest <- readConsecutive h (idx + 1) targetKey entrySize
                        return (entry : rest)
                    else return []

    parseEntry :: Get BookEntry
    parseEntry = do
        k <- getWord64be
        m <- getWord16be
        w <- getWord16be
        l <- getWord32be
        return $ BookEntry k m w l
