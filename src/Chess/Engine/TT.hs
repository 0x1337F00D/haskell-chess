...
-- Performance: Unboxing intermediate operations to eliminate GC-heavy thunk buildup in search paths.
{-# INLINE packData #-}
...
-- Inlined to allow GHC to specialize the `Maybe` sum type return at call sites,
-- preventing heap allocations in the hot search loop.
{-# INLINE probeTT #-}
...
-- Performance: Fold the upper 32 bits into the lower 32 bits before masking.
-- Inlined to float out unboxed strict data fields in AlphaBeta hot loops.
{-# INLINE storeTT #-}
