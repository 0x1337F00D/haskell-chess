...
alphaBetaRoot ctx vBoard tt depth nodes stopFlag limits = do
    ...
    let ttMove = if entryKey == hash
                 then let (m, _, _, _, _) = unpackData entryData in Just m
                 else Nothing
...
alphaBetaBody ctx vBoard tt lastMove depth alpha beta nodes stopFlag limits = do
    ...
    then return 0
        ...
        then return alpha'
                    ...
                    (entryKey, entryData) <- probeTTFast tt hash
                    let ttHitEntry = entryKey == hash
                    let (ttMove, ttScore, ttDepth, ttFlag) = if ttHitEntry
                            then let (m, s, d, f, _) = unpackData entryData in (Just m, s, d, f)
                            else (Nothing, 0, mkDepth (-1), TTExact)

                    let ttHit = ttHitEntry && ttDepth >= depth
