...
alphaBetaRoot ctx vBoard tt depth nodes stopFlag limits = do
    ...
    ttEntryRaw <- probeTTFast tt hash
    let ttMove = if ttEntryRaw /= 0
                 then let (m, _, _, _, _) = unpackData ttEntryRaw in Just m
                 else Nothing
...
alphaBetaBody ctx vBoard tt lastMove depth alpha beta nodes stopFlag limits = do
    ...
    then return 0
        ...
        then return alpha'
                    ...
                    ttEntryRaw <- probeTTFast tt hash
                    let (ttMove, ttScore, ttDepth, ttFlag) = if ttEntryRaw /= 0
                            then let (m, s, d, f, _) = unpackData ttEntryRaw in (Just m, s, d, f)
                            else (Nothing, 0, mkDepth (-1), TTExact)

                    let ttHit = ttEntryRaw /= 0 && ttDepth >= depth
