...
import Chess.Engine.TT (TT, cloneTT, probeTTFast, unpackData, storeTT, TTFlag(..))
...
alphaBetaRoot ctx vBoard tt depth nodes stopFlag limits = do
    ...
    let ttMove = if ttKey == hash then let (m, _, _, _, _) = unpackData ttData in Just m else Nothing
...
alphaBetaBody ctx vBoard tt lastMove depth alpha beta nodes stopFlag limits = do
    ...
    then return 0
        ...
        then return alpha'
                    ...
                    (ttKey, ttData) <- probeTTFast tt hash
                    let (ttMove, ttScore, ttDepth, ttFlag) = if ttKey == hash
                            then let (m, s, d, f, _) = unpackData ttData in (Just m, s, d, f)
                            else (Nothing, 0, mkDepth (-1), TTExact)

                    let ttHit = ttKey == hash && ttDepth >= depth
