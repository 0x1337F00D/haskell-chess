...
import Chess.Engine.TT (TT, cloneTT, probeTTFast, storeTT, TTFlag(..), unpackData)
...
alphaBetaBody ctx vBoard tt lastMove depth alpha beta nodes stopFlag limits = do
    ...
    then return 0
        ...
        then return alpha'
                    ...
                    (entryKey, entryData) <- probeTTFast tt hash
                    let ttHitRaw = entryKey == hash
                    let (ttMove, ttScore, ttDepth, ttFlag) = if ttHitRaw
                            then let (!m, !s, !d, !f, _) = unpackData entryData
                                 in (Just m, s, d, f)
                            else (Nothing, 0, mkDepth (-1), TTExact)

                    let ttHit = ttHitRaw && ttDepth >= depth
