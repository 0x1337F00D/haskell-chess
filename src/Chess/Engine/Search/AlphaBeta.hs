...
import Chess.Engine.TT (TT, cloneTT, probeTT, probeTTFast, unpackData, storeTT, TTFlag(..))
...
alphaBetaRoot ctx vBoard tt depth nodes stopFlag limits = do
    ...
    let ttMove = if ttEntryKey == hash
                 then let (!m, _, _, _, _) = unpackData ttEntryData in Just m
                 else Nothing
...
alphaBetaBody ctx vBoard tt lastMove depth alpha beta nodes stopFlag limits = do
    ...
    then return 0
        ...
        then return alpha'
                    ...
                    let (!ttMoveRaw, !ttScore, !ttDepth, !ttFlag, _) =
                            if ttHit then unpackData ttEntryData else (nullMove, 0, mkDepth (-1), TTExact, 0)
                    let ttMove = if ttHit then Just ttMoveRaw else Nothing

                    let ttHitDepth = ttHit && ttDepth >= depth
                    let ttCutoff = if ttHitDepth
                                   then case ttFlag of
                                       TTExact -> True
