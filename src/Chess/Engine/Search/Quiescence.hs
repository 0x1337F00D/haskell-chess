...
import Chess.Engine.TT (TT, probeTTFast, unpackScore, unpackFlag, storeTT, TTFlag(..))
...
quiescence ctx vBoard tt alpha beta nodes depth = do
    ...
    then do
        ...
        let staticEval = if ttData /= maxBound && unpackFlag ttData == TTEval
                         then Just (unpackScore ttData)
                         else Nothing
